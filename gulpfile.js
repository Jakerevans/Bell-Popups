...
var sassFrontendSource        = [ 'dev/scss/bellpopup-main-frontend.scss' ];
var sassBackendSource         = [ 'dev/scss/bellpopup-main-admin.scss' ];
...
gulp.task( 'concatAdminJs', function() {
	return gulp.src(jsBackendSource ) // use jsSources
		.pipe(concat( 'bellpopup_admin.min.js' ) ) // Concat to a file named 'script.js'
...
gulp.task( 'concatFrontendJs', function() {
	return gulp.src(jsFrontendSource ) // use jsSources
		.pipe(concat( 'bellpopup_frontend.min.js' ) ) // Concat to a file named 'script.js'
		.pipe(uglify() ) // Uglify concatenated file
		.pipe(gulp.dest( 'assets/js' ) ); // The destination for the concatenated and uglified file
});

gulp.task( 'copyassets', function () {
	return gulp.src([ './assets/**/*' ], {base: './'}).pipe(gulp.dest( '../bellpopup_dist/Bellpopup-Distribution' ) );
});

gulp.task( 'copyincludes', function () {
	return gulp.src([ './includes/**/*' ], {base: './'}).pipe(gulp.dest( '../bellpopup_dist/Bellpopup-Distribution' ) );
});

gulp.task( 'copyreadme', function () {
	return gulp.src([ './readme.txt' ], {base: './'}).pipe(gulp.dest( '../bellpopup_dist/Bellpopup-Distribution' ) );
});

gulp.task( 'copylang', function () {
	return gulp.src([ './languages/**/*' ], {base: './'}).pipe(gulp.dest( '../bellpopup_dist/Bellpopup-Distribution' ) );
});

gulp.task( 'copymainfile', function () {
	return gulp.src([ './bellpopup.php' ], {base: './'}).pipe(gulp.dest( '../bellpopup_dist/Bellpopup-Distribution' ) );
});

gulp.task( 'zip', function () {
	return gulp.src( '../bellpopup_dist/Bellpopup-Distribution/**' )
		.pipe(zip( 'bellpopup.zip' ) )
		.pipe(gulp.dest( '../bellpopup_dist/Bellpopup-Distribution' ) );
});

gulp.task( 'cleanzip', function(cb) {
	return del([ '../bellpopup_dist/Bellpopup-Distribution/**/*' ], {force: true}, cb);
});

gulp.task( 'clean', function(cb) {
	return del([ '../bellpopup_dist/Bellpopup-Distribution/**/*', '!../bellpopup_dist/Bellpopup-Distribution/bellpopup.zip' ], {force: true}, cb);
});
