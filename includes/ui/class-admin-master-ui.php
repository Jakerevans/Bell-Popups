...
if ( ! class_exists( 'BellPopup_Admin_Menu', false ) ) :
/**
 * BellPopup_Admin_Menu Class.
 */
class BellPopup_Admin_Menu {
    ...
    public function __construct() {
        ...
        switch ( $this->page ) {
            case 'BellPopUp-Options-popups':
                $this->setup_settings_ui();
                break; 
            case 'BellPopUp-Options-submenu-page1':
                $this->setup_submenupage1_ui();
                break;  
             case 'BellPopUp-Options-submenu-page2':
    ...
    private function setup_settings_ui() {
        $this->tabs = array(
            'popups1'   => __("Create Popups", 'bellpopupplugin'),
            'popups2'  => __("Edit & Delete Popups", 'bellpopupplugin'),
        );

        if(has_filter('bellpopup_add_tab_settings')) {
            $this->tabs = apply_filters('bellpopup_add_tab_settings', $this->tabs);
        }
    ...
    private function setup_submenupage1_ui() {
        $this->tabs = array(
            'submenupage1tab1'   => __("Submenu1 Tab 1", 'bellpopupplugin'),
            'submenupage1tab2'  => __("Submenu1 Tab 2", 'bellpopupplugin'),
        );

        if(has_filter('bellpopup_add_tab_submenupage1')) {
            $this->tabs = apply_filters('bellpopup_add_tab_submenupage1', $this->tabs);
        }
    ...
    private function setup_submenupage2_ui() {
        $this->tabs = array(
            'submenupage2tab1'   => __("Submenu2 Tab 1", 'bellpopupplugin'),
            'submenupage2tab2'  => __("Submenu2 Tab 2", 'bellpopupplugin'),
        );

        if(has_filter('bellpopup_add_tab_submenupage2')) {
            $this->tabs = apply_filters('bellpopup_add_tab_submenupage2', $this->tabs);
        }
    ...
    private function setup_dynamic_ui() {
        ...
        foreach($dir_array as $file){
            ...
            if($file == 'bellpopupplugin-'.$page[2].'.php'){
                continue;
            }
        ...
        foreach($tab_slug_array as $key=>$slug){
            $this->tabs[$slug] = __($tab_display_array[$key], 'bellpopupplugin');
        }

        // A filter to add tabs to the submenu page. So the submenu extensions can have their own separate plugins that add tabs to it. The name of this filter will be 'bellpopup_add_tab_' plus the one-word unique identifer for this extension, i.e., the word that is displayed in the BellPopup plugin main menu.  
        if(has_filter('bellpopup_add_tab_'.$page[2])) {
            $this->tabs = apply_filters('bellpopup_add_tab_'.$page[2], $this->tabs);
        }
    ...
    private function output_indiv_tab() {
        ...
        if(!file_exists(BELLPOPUP_ROOT_INCLUDES_UI_ADMIN_DIR.$filename)){
            $path = $this->build_extension_path();
            if(is_dir($path)){
           ...
           require_once(BELLPOPUP_ROOT_INCLUDES_UI_ADMIN_DIR.$filename);
        }
    }
    ...
    private function build_extension_path() {
        ...
        foreach($this->active_plugins as $plugin){
            if(strpos($plugin, 'bellpopupplugin-') !== false){
                ...
                if(file_exists($path) && !is_dir($path)){
                    ...
                    if(strpos($plugin, $page[2]) !== false){
                        $path = ROOT_WP_PLUGINS_DIR.'bellpopupplugin-'.$page[2];
                        if(file_exists($path)){
                            return $path;
                        }
                    }
